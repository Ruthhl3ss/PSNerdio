function Get-NerdioMEHostpoolActiveDirectoryConfig {
  <#
  .SYNOPSIS
    Retrieves the Active Directory configuration for a specific Nerdio Managed Environment (NME) host pool.
  .DESCRIPTION
    The Get-NerdioMEHostpoolActiveDirectoryConfig cmdlet fetches information about the Active Directory configuration of a specified host pool within a Nerdio Managed Environment (NME).
  ...
  begin {
    Write-Verbose "Starting Get-NerdioMEHostpoolActiveDirectoryConfig for host pool: $Name in resource group: $ResourceGroupName under subscription: $SubscriptionId"
    Get-TokenValidity
  }
  process {

    $uri = "$script:NMEBaseurl/api/$Script:NMEApiVersion/arm/hostpool/$subscriptionId/$ResourceGroupName/$Name/active-directory"
    Write-Verbose "Constructed URI: $uri"

    try {
      $Hostpool = Invoke-RestMethod -Method GET -Uri $uri -Headers $script:NMEAuthheader
      Write-Verbose "Successfully retrieved host pool active directory details."
